...
from gamesimulation import Simulator, simulate
import json
import numpy
import rel
import websocket
...
def load_model():
    if os.path.exists(MODELPATH):
        savedModel = load_model(MODELPATH)
    else:
        savedModel = simulate()
...
def get_best_move_pgn(bagchal: Bagchal):
    ...
    prediction = savedModel.predict_on_batch(inputs)

    best_move_index = prediction.argmax()
